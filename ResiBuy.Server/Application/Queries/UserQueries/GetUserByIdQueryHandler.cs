namespace ResiBuy.Server.Application.Queries.UserQueries
{
    ...
    public class GetUserQueryHandler(IUserDbService UserDbService) : IRequestHandler<GetUserByIdQuery, ResponseModel>
    {
        public async Task<ResponseModel> Handle(GetUserByIdQuery query, CancellationToken cancellationToken)
        {
            ...
            return ResponseModel.SuccessResponse(new UserQueryResult(user.Id, user.Email, user.DateOfBirth, user.IsLocked, user.Roles, user.FullName,
                user.CreatedAt, user.UpdatedAt, user.Cart.Id, user.Avatar != null ? new AvatarQueryResult(user.Avatar.Id, user.Avatar.Name, user.Avatar.ImgUrl, user.Avatar.ThumbUrl) : null, user.UserRooms.Select(ur => new RoomQueryResult(ur.RoomId, ur.Room.Name, ur.Room.Building.Name, ur.Room.Building.Area.Name)),
