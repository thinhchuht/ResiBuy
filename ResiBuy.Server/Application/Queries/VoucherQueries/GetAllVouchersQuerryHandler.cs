namespace ResiBuy.Server.Application.Queries.VoucherQueries
{
    ...
    public class GetAllVouchersQueryHandler(IVoucherDbService voucherDbService) : IRequestHandler<GetAllVouchersQuery, ResponseModel>
    {
        public async Task<ResponseModel> Handle(GetAllVouchersQuery query, CancellationToken cancellationToken)
        {
            ...
            var pagedResult = await voucherDbService.GetAllVouchersAsync(query.VoucherDto.StoreId, query.VoucherDto.IsActive, query.VoucherDto.IsGettingNow, query.VoucherDto.StartDate, query.VoucherDto.EndDate, query.VoucherDto.PageNumber, 
                query.VoucherDto.PageSize, query.VoucherDto.UserId, query.VoucherDto.MinOrderPrice, query.VoucherDto.Type);
