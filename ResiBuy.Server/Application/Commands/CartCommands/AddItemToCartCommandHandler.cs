...
namespace ResiBuy.Server.Application.Commands.CartCommands
{
    ...
    public record AddItemToCartCommand(Guid Id, AddToCartDto AddToCartDto) : IRequest<ResponseModel>;
    public class AddItemToCartCommandHandler(ICartItemDbService cartItemDbService, ICartDbService cartDbService,
        IProductDetailDbService baseProductDbService
        ) : IRequestHandler<AddItemToCartCommand, ResponseModel>
    {
        public async Task<ResponseModel> Handle(AddItemToCartCommand command, CancellationToken cancellationToken)
        {
            try
            {
