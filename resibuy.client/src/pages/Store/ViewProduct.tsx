...
export default function ViewProduct() {
    ...
    const handlePrintBarcodes = (productDetail: ProductDetailInput) => {
        const validBarcodes = productDetail.barcodes.filter(barcode => barcode.trim() !== '');
        ...
        validBarcodes.forEach((barcode, index) => {
            ...
            if (imageUrl) {
                ...
                // Center the barcode image on the page (A4: 210mm x 297mm)
                doc.addImage(imageUrl, 'PNG', 55, 100, 100, 30); // Centered at x=55 (210-100)/2
            }
        });
                                    ...
                                    {selectedProductDetail.barcodes.map((barcode, index) => (
                                        <Box key={index} sx={{ border: "1px solid #e0e0e0", borderRadius: 2, p: 2 }}>
                                            ...
                                            {generateBarcodeImage(barcode) ? (
                                                <img
                                                    src={generateBarcodeImage(barcode)}
                                                    alt={`Barcode ${barcode}`}
                                                    style={{
                                                        width: "100%",
                                                        maxWidth: 300,
                                                ...
                                                <Typography color="error">Không thể tạo mã vạch: {barcode}</Typography>
                                            )}
                                            <Button
                                                variant="outlined"
