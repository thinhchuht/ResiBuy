...
const ProductPage: React.FC = () => {
    ...
    // State cho menu dropdown của nút thêm sản phẩm
    ...
    const handleExcelUpload = async (file: File) => {
        ...
        try {
            ...
            // Gọi API upload file Excel
            const response = await axios.post('/api/Product/import-excel', formData, {
                headers: {
            ...
            if (response.data.success) {
                toast.success(`Đã thêm thành công ${response.data.addedCount} sản phẩm từ file Excel`);
                // Làm mới danh sách sản phẩm
                fetchProducts();
            } else {
                toast.error(response.data.message || 'Có lỗi xảy ra khi xử lý file Excel');
            }
        } catch (error: any) {
            console.error('Lỗi khi upload file Excel:', error);
            const errorMessage = error.response?.data?.message || 'Có lỗi xảy ra khi upload file Excel';
            ...
            <Paper
                ...
                <Box display="flex" justifyContent="space-between" alignItems="center" flexWrap="wrap" gap={2}>
                    ...
                    <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
                        {/* Loading indicator khi upload Excel */}
                        {isUploadingExcel && (
                        ...
                        <Menu
                            ...
                            <MenuItem
                                ...
                                <CloudUploadIcon sx={{ mr: 2, color: 'success.main' }} />
                                <Box>
                                    <Typography variant="body1" sx={{ fontWeight: 600 }}>
                                        Thêm bằng Excel
                                    </Typography>
                                    <Typography variant="body2" color="text.secondary">
                                        Nhập nhiều sản phẩm từ file Excel
                                    </Typography>
                                </Box>
                            </MenuItem>
                        </Menu>
