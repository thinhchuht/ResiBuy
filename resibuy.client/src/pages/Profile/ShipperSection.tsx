...
import { LocalShipping, LocationOn, CalendarMonth, Assignment, ReportProblem } from "@mui/icons-material";
...
const ShipperSection = () => {
  ...
  return (
    <>
      <Paper elevation={0} sx={{ p: { xs: 2, md: 4 }, borderRadius: 3, bgcolor: "#fff", boxShadow: "0 2px 8px rgba(233,30,99,0.10)", maxWidth: 700, mx: "auto", border: '1.5px solid #f8bbd0' }}>
        ...
        <Box display="flex" flexDirection={{ xs: "column", md: "row" }} gap={4} alignItems={{ md: "center" }}>
          <Box flex={1} minWidth={0}>
            <Box display="flex" alignItems="center" mb={1}>
              ...
              <Typography variant="body2">Số đơn đã giao: <b>{shipper.orders?.length ?? 0}</b></Typography>
            </Box>
            <Box display="flex" alignItems="center" mb={1}>
              <ReportProblem sx={{ mr: 1, color: "#e91e63" }} />
              <Typography variant="body2">Số lần bị báo cáo: <b>{shipper.reportCount}</b></Typography>
            </Box>
            <Box display="flex" alignItems="center" mb={1}>
              <CalendarMonth sx={{ mr: 1, color: "#e91e63" }} />
              <Typography variant="body2">Bắt đầu ca: <b>{floatToTime(shipper.startWorkTime)}</b></Typography>
            </Box>
            <Box display="flex" alignItems="center" mb={1}>
              <CalendarMonth sx={{ mr: 1, color: "#e91e63" }} />
              <Typography variant="body2">Kết thúc ca: <b>{floatToTime(shipper.endWorkTime)}</b></Typography>
            </Box>
            <Box display="flex" alignItems="center" mb={1}>
              <LocationOn sx={{ mr: 1, color: "#e91e63" }} />
              <Typography variant="body2">Vị trí hiện tại: <b>{location}</b></Typography>
            </Box>
          </Box>
          <Box display={{ xs: 'none', md: 'flex' }} flexDirection="column" alignItems="center" justifyContent="center" sx={{ minWidth: 140 }}>
            <Box sx={{ bgcolor: '#fce4ec', borderRadius: '50%', width: 100, height: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 12px rgba(233,30,99,0.12)', mb: 2 }}>
