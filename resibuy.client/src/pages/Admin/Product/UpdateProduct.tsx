...
const validateQuantity = (
    ...
    index: number,
    isNewDetail: boolean,
    currentBarcodes: { code: string; orderItemId?: string }[]
  ) => {
    ...
    if (quantity < 0) {
      ...
      const nonSoldBarcodes = currentBarcodes.filter(b => !b.orderItemId).length;
      if (quantity < nonSoldBarcodes) {
        setSelectedDetailIndex(index);
        setIsNewDetail(isNewDetail);
        setDeleteCount(nonSoldBarcodes - quantity);
        setBarcodesToDelete([]);
