...
const reportApi = {
  getAll: async (params: { userId?: string; keyword?: string; startDate?: string; endDate?: string; reportStatus?: string; pageNumber?: number; pageSize?: number }) => {
  ...
  getCount: async (params?: { startDate?: string; endDate?: string }) => {
    const response = await axiosClient.get("/api/report/count", { params });
    return response.data;
  },
  create: async (data: { orderId: string; userId: string; targetId: string; title: string; description: string; reportTarget: number }) => {
