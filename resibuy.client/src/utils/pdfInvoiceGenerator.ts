...
export const generateInvoicePDF = (orderData: OrderData): void => {
  ...
  const addText = (
    ...
    x: number,
    y: number,
    options: { fontSize?: number; fontStyle?: string; color?: number | number[]; align?: "left" | "right" | "center" } = {}
  ) => {
  ...
  orderData.items.forEach((item, index) => {
    ...
    if (item.productDetail?.additionalData && item.productDetail.additionalData.length > 0) {
      const attrs = item.productDetail.additionalData
        .map((ad) => `${ad.key}: ${ad.value}`)
        .join(" | ");
  ...
  addText("Tạm tính:", 30, yPosition, { fontSize: 10 });
  addText(
    formatCurrency(orderData.items.reduce((sum, item) => sum + item.price * item.quantity - (item.discount || 0) * item.quantity, 0)),
    pageWidth - 30,
    yPosition,
    {
      fontSize: 10,
  ...
  const finalTotal = orderData.total + (orderData.shippingFee || 0) - orderData.discount - (orderData.voucherDiscount || 0);
  ...
  // Payment info
  if (orderData.paymentMethod === "COD" && orderData.customerPaid) {
    addText("Phương thức thanh toán: Tiền mặt", 20, yPosition, { fontSize: 10 });
    yPosition += 6;
    addText(`Tiền khách đưa: ${formatCurrency(orderData.customerPaid)}`, 20, yPosition, { fontSize: 10 });
    if (orderData.change && orderData.change > 0) {
      yPosition += 6;
      addText(`Tiền thừa trả khách: ${formatCurrency(orderData.change)}`, 20, yPosition, { fontSize: 10 });
    }
  } else {
